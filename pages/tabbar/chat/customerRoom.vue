<template>
	<view >
		<cu-custom bgColor="bg-white" :isBack="true" backColor="text-red">
				<block slot="content" style="font-weight: bold;" >
					{{userName}}
				</block>
		</cu-custom>
		<view class="content-c" @touchstart="hideDrawer">
				...
				<view class="row" v-for="(item, index) in msgList" :key="index" :id="item.id">
					...
					<block>
						...
						<view class="other" v-else>
							...
							<view class="right">
								<view class="username">
									<view class="name">{{item.nick || '用户' + item.from }}</view>
		...
		<view class="input-box-s" :class="popupLayerClass" @touchmove.stop.prevent="discard">
			...
			<view class="textbox">
				...
				<view class="text-mode"  :class="isVoice?'hidden':''">
					<view class="box">
						<textarea adjust-position="false" auto-height="true" cursor-spacing='10' v-model="textMsg" placeholder="来,想说点什么吗？" placeholder-style="font-size:13px" @focus="textareaFocus"/>
					</view>
